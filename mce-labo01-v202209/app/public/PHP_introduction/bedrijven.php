...
if(array_key_exists('id', $_GET)){
    if(array_key_exists($_GET['id'], $companies)){
        $companies = [$companies[$_GET['id']]];
    }
}

/**
 * http://localhost:8080/labo02/bedrijven.php?prefix=9
 */
if(array_key_exists('prefix', $_GET)){
    $prefix = $_GET['prefix'];
    $result = [];
    foreach($companies as $company){
        if(((string )$company['zip'])[0] === $prefix){
            $result[] = $company;
        }
    }
...
if(array_key_exists('sort', $_GET)){
    ...
    $companies = $result;
    $keys = array_column($companies, 'zip');
    array_multisort($keys, SORT_ASC, $companies);
